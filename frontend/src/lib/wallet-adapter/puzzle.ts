...
import {CreateEventRequestData} from "@puzzlehq/sdk/src/messages/createEvent";

export interface PuzzleWalletEvents {
...
export class PuzzleWalletAdapter extends BaseMessageSignerWalletAdapter {
    ...
    private _wallet: WalletConnectModalSign | null;
    ...
    constructor({ appName = 'sample' }: PuzzleWalletAdapterConfig = {}) {
        ...
        if (this._readyState !== WalletReadyState.Unsupported) {
            getWalletConnectModalSignClient().then((wallet) => {
                this._wallet = wallet;
            });
        }
    }
    ...
    async connect(decryptPermission: DecryptPermission, network: WalletAdapterNetwork): Promise<void> {
        try {
            ...
            try {
                ...
                this._publicKey = session.self.publicKey!;
            } catch (error: any) {
    ...
    async signMessage(message: Uint8Array): Promise<Uint8Array> {
        try {
            ...
            try {
                const signature = await requestSignature({message, address: this.publicKey});
                if (signature.error) {
